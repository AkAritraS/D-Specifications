# --- Delta DOS-Style Device Info & Booster (with typing bar + commands) ---
...
class DeltaDOSApp(ctk.CTk):
    ...
    def _create_layout(self):
        ...
        console_frame.pack(fill="both", expand=True, padx=8, pady=(0,6))
        ...
        # --- Typing Bar Added Here ---
        self.cmd_entry = ctk.CTkEntry(left, placeholder_text="Type command here (help for list)...", font=("Consolas", 12))
        self.cmd_entry.pack(fill="x", padx=8, pady=(0, 8))
        ...
        # Right side panels remain the same ↓ (no change)
    ...
    def _handle_command(self, event=None):
        ...
        if not cmd:
            return
        self._typing_effect(f"> {cmd}", delay=0.001)

        if cmd == "help":
            help_text = (
                "\nAvailable commands:\n"
                " help     - Show this help menu\n"
                " specs    - Display system specifications\n"
                " boost    - Clear temp & flush DNS\n"
                " clear    - Clear console\n"
                " exit     - Exit DeltaDOS\n"
            )
            self._typing_effect(help_text, delay=0.004)

        elif cmd == "specs":
            self._demo_type_specs()

        elif cmd == "boost":
            self._typing_effect("Running full boost...\n")
            threading.Thread(target=lambda: [self._typing_effect(clear_temp_files()), self._typing_effect(flush_dns())], daemon=True).start()

        elif cmd == "clear":
            self._clear_console()
            self._typing_effect("Console cleared.\n")

        elif cmd == "exit":
            self._typing_effect("Exiting DeltaDOS...\n")
            self.after(1000, self.destroy)

        else:
            self._typing_effect(f"Bad command or file name: {cmd}\n")
